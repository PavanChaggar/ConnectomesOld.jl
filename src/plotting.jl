...
function plot_connectome(connectome::Connectome; node_size::Float64=1.0)
    x, y, z = connectome.parc.x[:], connectome.parc.y[:], connectome.parc.z[:]

    coordindex = findall(x->x>0, connectome.A)
    
    D = [connectome.D[i,i] for i in 1:length(connectome.parc.ID)]
    D_scale = D ./ maximum(D)
    
    fig = plot_mesh(;alpha=0.1, transparent=true)
    meshscatter!(x, y, z, markersize=D_scale*node_size, color=(:blue,0.5))
